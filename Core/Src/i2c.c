...
/* USER CODE BEGIN 0 */
#define MPU6050_SLAVE_ADDRESS      0x33  
HAL_StatusTypeDef I2C_MPU6050_ReadBuffer(uint16_t Addr, uint8_t Reg, uint16_t RegSize, uint8_t *pBuffer, uint16_t Length)
{
  ...
  status = HAL_I2C_Mem_Read(&hi2c1, Addr, (uint16_t)Reg, RegSize, pBuffer, Length, 10);
  
  /* ?¨¬2aI2C¨ª¡§D?¡Á¡ä¨¬? */
  if(status != HAL_OK)
  {
    /* ¦Ì¡Â¨®?I2C¨ª¡§D?¡ä¨ª?¨®¡ä|¨¤¨ªo¡¥¨ºy */
    while(1);
  }        
  return status;
}

void MEMSReadData(uint8_t reg_add,unsigned char *Read,uint8_t num)
{
