...
#define STATE_AT_RINGBUF_OVERRUN                                   (-0x1001)
#define STATE_AT_LOG_RINGBUF_OVERRUN                               (-0x1002)
#define STATE_AT_NO_HANDLE_EXIST                                   (-0x1003)
...
/**
 * @brief SDK从ringbuf中读取数据前，需要disable串口中断
 * 
 * @details
 * 
 * SDK从从ringbuf中读取数据前，需要disable串口中断，避免SDK在读取数据过程中产生中断往ringbuf中
 * 写入数据时损坏ringbuf
 * 
 * @return void
 * 
 */
typedef void (*aiot_at_disable_uart_interrupt_handler_t)(void);

/**
 * @brief SDK从ringbuf中读取数据后，需要enable串口中断
 * 
 * @details
 * 
 * SDK从从ringbuf中读取数据后，恢复串口中断，使串口可以正常接收数据
 * 
 * @return void
 * 
 */
typedef void (*aiot_at_enable_uart_interrupt_handler_t)(void);
...
typedef struct {
     ...
     * @brief SDK从ringbuf中读取数据前，需要disable串口中断
     */
    aiot_at_disable_uart_interrupt_handler_t disable_uart_interrupt_handler;

    /**
     * @brief SDK从ringbuf中读取数据后，需要enable串口中断
     */
    aiot_at_enable_uart_interrupt_handler_t enable_uart_interrupt_handler;
} aiot_at_send_handler_t;

/**
 * @brief 设置SDK需要的发送回调函数已经中断管理函数
 *
 ...
 * @details
 *
 * 此函数应在portfile的 @ref aiot_sysdep_portfile_t::core_sysdep_network_init 函数中调用, 初始化at句柄
 *
 * @return void*
 *
 * @retval 非NULL 初始化成功
 ...
 * @details
 *
 * 此函数应在portfile的 @ref aiot_sysdep_portfile_t::core_sysdep_network_establish, @ref aiot_sysdep_portfile_t::core_sysdep_network_send
 *
 * 或 @ref aiot_sysdep_portfile_t::core_sysdep_network_deinit 中调用, 分别用于建立网络连接/发送网络数据和断开网络连接
 *
 * @param handle at句柄
 ...
 * @details
 *
 * 此函数应在portfile的 @ref aiot_sysdep_portfile_t::core_sysdep_network_establish, @ref aiot_sysdep_portfile_t::core_sysdep_network_recv
 *
 * 或 @ref aiot_sysdep_portfile_t::core_sysdep_network_deinit 中调用, 分别用于接收建立网络连接的应答/接收网络数据和接收断开网络连接的应答
 *
 * @param handle at句柄
 ...
 * @details
 *
 * 此函数应在portfile的 @ref aiot_sysdep_portfile_t::core_sysdep_network_deinit 中调用, 用于销毁at句柄
 *
