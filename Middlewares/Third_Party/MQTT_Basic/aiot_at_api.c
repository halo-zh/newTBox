...
static int32_t _core_at_find_handle_by_socket_id(char *socket_id, core_at_handle_t **at_handle)
{
    ...
    if (&node->linked_node == &g_at_global.at_handle_list) {
        return STATE_AT_NO_HANDLE_EXIST;
    }
...
int32_t aiot_at_set_send_handler(aiot_at_send_handler_t *handler)
{
    ...
    g_at_global.disable_uart_interrupt_handler = handler->disable_uart_interrupt_handler;
    g_at_global.enable_uart_interrupt_handler = handler->enable_uart_interrupt_handler;
...
int32_t aiot_at_recv(void *handle, aiot_at_recv_option_t option, void *data)
{
    ...
    if (g_at_global.disable_uart_interrupt_handler != NULL) {
        g_at_global.disable_uart_interrupt_handler();
    }
    ...
    if (g_at_global.enable_uart_interrupt_handler != NULL) {
        g_at_global.enable_uart_interrupt_handler();
    }
