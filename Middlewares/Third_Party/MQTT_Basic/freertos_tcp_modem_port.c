...
#define AT_RINGBUF_ACCESS_INTERVAL_MS   (120)
...
static void _core_sysdep_network_tcp_disconnect(core_network_handle_t *network_handle)
{
    ...
    res = aiot_at_send(network_handle->at_handle, AIOT_ATSENDOPT_DISCONNECT_REQ, NULL);
    if (res < STATE_SUCCESS) {
        return;
    }
    ...
    if (res < STATE_SUCCESS) {
        // PRINTF("aiot_at_recv failed: %d\r\n", res);
    }

    if (result == 0) {
        // PRINTF("aiot_at_recv wait socket close failed\r\n");
    }
